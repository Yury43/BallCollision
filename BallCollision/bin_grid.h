...
{
public:
    BinGrid(const float max_span, std::vector<std::shared_ptr<T>>& objects) :
        bin_size( max_span * 2),
        ...
        n_bins_y( static_cast<int>(std::ceil(WINDOW_Y / bin_size)) + 2)
    {
        ...
        // Place balls in bins, delete those that escaped 
        {
            ...
            while (oit != objects.end())
            {
                ...
                {
                    std::cout << "Ball " << (*oit)->id << " " << (*oit)->p << " escaped to [" << bin_x << ", " << bin_y << "] !" << std::endl;
    ...
    void detect_collisions(const std::shared_ptr<T> & that, std::vector<std::pair<int, int>> & colliding_pairs) const 
    {
